...
try:
    ...
    for keyword_to_search in keywords_to_search:
        ...
        for button in buttons:
            ...
            for row in rows:
                ...
                tabs_data[button] = table_data
            all_data.append({keyword_to_search:tabs_data})
            pprint(all_data)
            continue
            df = pd.DataFrame(table_data)
            df.to_csv(f"{keyword_to_search}-{button}-vidiq-results-25-12-2024.csv")
            df.to_excel(f"{keyword_to_search}-{button}-vidiq-results-25-12-2024.xlsx")


            # Print the extracted table data as a list of objects
            with open(f"{keyword_to_search}-{button}-vidiq-results-25-12-2024.csv", "w", encoding="utf-8") as file:
    ...
    df.to_excel("vidiq-data-26-12-2024.xlsx")
    # pprint("="*50)
    # pprint(table_data)
    # pprint("="*50)
